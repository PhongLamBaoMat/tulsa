...
class CyfirmaComSpider(Spider):
    ...
    async def run(self) -> FinalStatistics:  # pyright: ignore [reportIncompatibleMethodOverride]
        return await super().run(
            [
                "https://www.cyfirma.com/cyber-research/",
                "https://www.cyfirma.com/whitepapers/",
            ]
        )
    ...
    async def default_request_handler(context: ParselCrawlingContext):
        items = context.selector.xpath(
            '//div[@class="col-12 col-lg-6 col-md-6 blogs mb-4"]'
        )
        ...
        for entry in items:
            ...
            description = entry.xpath(".//p/text()").get()
            thumbnail = entry.xpath(".//img/@src").get()
            published = entry.xpath(".//date/text()").get()
