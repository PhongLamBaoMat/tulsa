...
class TrmlabsComSpider(Spider):
    def __init__(self) -> None:
        super().__init__(default_request_handler=self.default_request_handler)

    @override
    async def run(self) -> FinalStatistics:  # pyright: ignore [reportIncompatibleMethodOverride]
        return await super().run(["https://www.trmlabs.com/category/insights"])

    @staticmethod
    async def default_request_handler(context: ParselCrawlingContext):
        items = context.selector.xpath(
            '//div[@role="listitem" and @class="list-item w-dyn-item"]'
        )
        ...
        for entry in items[:20]:
            ...
            if not title:
                ...
                continue
            url = entry.xpath("./div/a/@href").get()
            if not url:
                context.log.error(
            ...
            published = entry.xpath('.//div[@class="subtitle"]/text()').get()
