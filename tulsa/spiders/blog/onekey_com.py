...
class OnekeyComSpider(Spider):
    def __init__(self) -> None:
        super().__init__(default_request_handler=self.default_request_handler)

    @override
    async def run(self) -> FinalStatistics:  # pyright: ignore [reportIncompatibleMethodOverride]
        return await super().run(["https://www.onekey.com/resources/research"])

    @staticmethod
    async def default_request_handler(context: ParselCrawlingContext):
        items = context.selector.xpath('//div[@hover="parent" and @role="listitem"]')
        ...
        for entry in items:
            ...
            published = entry.xpath(
                './/div[@class="featured-rsrc_item_tag text-size-regular text-style-tag text-weight-bold"]/text()'
            )[1].get()
            description = entry.xpath(
                './/p[@class="featured-rsrc_item_paragraph text-size-regular text-color-2"]/text()'
            ).get()
